<template>
  <div>
    ...
    <div class="calendar-grid">
      ...
      <div class="calendar-header">
        <div
          ...
          :key="day"
          class="day-header text-caption text-medium-emphasis text-center"
        >
          {{ day }}
        </div>
      </div>
      ...
      <div v-for="(week, wi) in visibleWeeks" :key="wi" class="calendar-week">
        <div
          ...
          :class="{
            ...
            'day-past': day.isPast,
            'day-available':
              day.isCurrentUserAvailable && !day.isCurrentUserMaybe,
            'day-maybe': day.isCurrentUserMaybe,
            'day-qualified':
              day.meetsThreshold && !day.isConfirmed && !day.isCancelled,
          ...
          @click="
            !day.isPast && !day.isBlocked && toggleAvailability(day.dateStr)
          "
        >
          <div class="day-number text-caption">{{ day.dayNum }}</div>
          <div
            class="day-month text-caption text-medium-emphasis"
            v-if="day.showMonth"
          >
            {{ day.monthStr }}
          </div>
          ...
          <v-icon
            v-if="day.isConfirmed && !day.isCancelled"
            color="success"
            size="small"
          ...
          <div
            v-if="(day.isConfirmed || day.isCancelled) && day.confirmedEntry"
            class="confirmed-details text-caption"
          >
            <span
              v-if="day.confirmedEntry.location"
              class="d-block text-truncate"
            >
              <v-icon size="x-small" class="mr-1">mdi-map-marker</v-icon
              >{{ day.confirmedEntry.location }}
            </span>
            <span v-if="day.confirmedEntry.startTime" class="d-block">
              <v-icon size="x-small" class="mr-1">mdi-clock</v-icon
              >{{ day.confirmedEntry.startTime
              }}<template v-if="day.confirmedEntry.endTime">
                – {{ day.confirmedEntry.endTime }}</template
              >
            </span>
            <span
              v-if="day.confirmedEntry.rescheduledFrom"
              class="d-block text-caption text-warning"
            >
              Verschoben
            </span>
          </div>
          ...
          <div
            class="avatar-stack"
            v-if="day.availableUsers.length > 0 || day.maybeUsers.length > 0"
          >
            <UserAvatar
          ...
          <v-icon
            v-if="
              day.requiredBlocked &&
              (day.availableUsers.length > 0 || day.maybeUsers.length > 0) &&
              !day.isPast
            "
            color="error"
            size="x-small"
          ...
          <v-btn
            ...
            :title="
              day.requiredBlocked
                ? 'Pflichtmitglied nicht verfügbar'
                : undefined
            "
          ...
          <template
            v-if="
              canConfirm && day.isConfirmed && !day.isCancelled && !day.isPast
            "
          >
            <v-btn
              size="x-small"
    ...
    <div class="d-flex flex-wrap gap-3 mt-4">
      ...
      <div
        v-if="requiredParticipantIds.length > 0"
...
const visibleWeeks = computed(() => {
  ...
  for (let w = 0; w < numWeeks; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const availableUsers = props.availabilities.filter((a) =>
        a.dates.includes(dateStr),
      );
      const maybeUsers = props.availabilities.filter((a) =>
        a.maybeDates?.includes(dateStr),
      );
      const isCurrentUserAvailable = availableUsers.some(
        (a) => a.userId === props.currentUserId,
      );
      const isCurrentUserMaybe =
        !isCurrentUserAvailable &&
        maybeUsers.some((a) => a.userId === props.currentUserId);

      const confirmedEntry = props.confirmedDates.find(
        (c) => c.date === dateStr,
      );
      ...
      const requiredMissing = props.requiredParticipantIds.filter(
      ...
      const effectiveCount =
        ...
        0.5 *
          maybeUsers.filter((u) => !availableUserIds.includes(u.userId)).length;
      const meetsThreshold =
        !requiredBlocked && effectiveCount >= props.minParticipants;
      ...
      week.push({
        ...
        requiredBlocked,
        availableUsers,
        maybeUsers: maybeUsers.filter(
          (u) => !availableUserIds.includes(u.userId),
        ),
        isBlocked,
        isHoliday,
        holidayName,
...
function handleConfirmDialogSave(data: {
  date: string;
  location: string;
  startTime: string;
  endTime: string;
